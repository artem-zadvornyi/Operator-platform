...
async def create_company(
    ...
    try:
        return await orchestrator.create_company(payload.idea)
...
async def start_company(
    ...
    try:
        return await orchestrator.start_company(company_id)
...
async def get_company(
    ...
    try:
        return await orchestrator.get_company(company_id)
    except CompanyNotFoundError as error:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=str(error),
